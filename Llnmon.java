...
public class Llnmon {
    ...
    private static String convertToMixed(int addressLow, int addressHigh) {
        if (addressHigh == 0) {
            if (addressLow >= 120) return "c" + String.valueOf(addressLow - 120) + " (" + String.valueOf(addressLow) + ")"; else if (addressLow >= 110) return "b" + String.valueOf(addressLow - 110) + " (" + String.valueOf(addressLow) + ")"; else if (addressLow >= 100) return "a" + String.valueOf(addressLow - 100) + " (" + String.valueOf(addressLow) + ")"; else return String.valueOf(addressLow & 0x7f);
        } else {
    ...
    public static String getDeviceNameFromIPLInfo(int manuf, int type) {
        return ((manuf != LnConstants.RE_IPL_MFR_DIGITRAX) ? "Unknown manufacturer " + manuf + ", unknown device " + type : (type == LnConstants.RE_IPL_DIGITRAX_HOST_ALL) ? "Digitrax (no host device type specified)" : (type == LnConstants.RE_IPL_DIGITRAX_HOST_UT4) ? "Digitrax UT4(x) host" : (type == LnConstants.RE_IPL_DIGITRAX_HOST_UR92) ? "Digitrax UR92 host" : (type == LnConstants.RE_IPL_DIGITRAX_HOST_DT402) ? "Digitrax DT402(x) host" : (type == LnConstants.RE_IPL_DIGITRAX_HOST_DCS51) ? "Digitrax DCS51 host" : (type == LnConstants.RE_IPL_DIGITRAX_HOST_PR3) ? "Digitrax PR3 host" : "Digitrax (unknown host device type)");
    }

    public static String getSlaveNameFromIPLInfo(int manuf, int slaveNum) {
        return ((manuf != LnConstants.RE_IPL_MFR_DIGITRAX) ? "Unknown manufacturer " + manuf + ", unknown device " + slaveNum : (slaveNum == LnConstants.RE_IPL_DIGITRAX_SLAVE_ALL) ? "Digitrax (no slave device type specified)" : (slaveNum == LnConstants.RE_IPL_DIGITRAX_SLAVE_RF24) ? "Digitrax RF24 slave" : "Digitrax (unknown slave device type)");
    }
